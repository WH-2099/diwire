...
def test_benchmark_diwire_resolve_scoped_with_registered_open_closed_generics_pair_same(
    benchmark: Any,
) -> None:
    ...
    with container.enter_scope(Scope.REQUEST) as bench_scope:
        ...
        def bench_diwire_resolve_scoped_with_registered_open_closed_generics_pair_same() -> None:
            _ = bench_scope.resolve(repo_int)
            _ = bench_scope.resolve(repo_int)

        run_benchmark(
            benchmark,
            bench_diwire_resolve_scoped_with_registered_open_closed_generics_pair_same,
        )


def test_benchmark_dishka_resolve_scoped_with_registered_open_closed_generics_pair_same(
    benchmark: Any,
) -> None:
    ...
    with container(scope=DishkaBenchmarkScope.REQUEST) as bench_scope:
        ...
        def bench_dishka_resolve_scoped_with_registered_open_closed_generics_pair_same() -> None:
            _ = bench_scope.get(repo_int)
            _ = bench_scope.get(repo_int)

        run_benchmark(
            benchmark,
            bench_dishka_resolve_scoped_with_registered_open_closed_generics_pair_same,
        )
