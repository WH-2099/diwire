from __future__ import annotations

from dataclasses import dataclass
from typing import Any, Generic, TypeVar, cast

from dishka import Provider

from diwire import Lifetime, Scope
from tests.benchmarks.dishka_helpers import DishkaBenchmarkScope, make_dishka_benchmark_container
from tests.benchmarks.helpers import make_diwire_benchmark_container, run_benchmark

T = TypeVar("T")


class _Repo(Generic[T]):
    pass


@dataclass
class _GenericRepo(_Repo[T]):
    dependency_type: type[T]


def test_benchmark_diwire_enter_close_scope_resolve_open_generic_scoped(benchmark: Any) -> None:
    container = make_diwire_benchmark_container()
    container.add(
        _GenericRepo,
        provides=_Repo,
        lifetime=Lifetime.SCOPED,
        scope=Scope.REQUEST,
    )
    container.compile()

    with container.enter_scope(Scope.REQUEST) as first_scope:
        first_int = cast("Any", first_scope.resolve(_Repo[int]))
        second_int = cast("Any", first_scope.resolve(_Repo[int]))
        first_str = cast("Any", first_scope.resolve(_Repo[str]))
    with container.enter_scope(Scope.REQUEST) as second_scope:
        third_int = cast("Any", second_scope.resolve(_Repo[int]))
    assert isinstance(first_int, _GenericRepo)
    assert isinstance(second_int, _GenericRepo)
    assert isinstance(first_str, _GenericRepo)
    assert isinstance(third_int, _GenericRepo)
    assert first_int is second_int
    assert first_int is not third_int
    assert first_int is not first_str
    assert first_int.dependency_type is int
    assert first_str.dependency_type is str
    assert third_int.dependency_type is int

    def bench_diwire_enter_close_scope_resolve_open_generic_scoped() -> None:
        with container.enter_scope(Scope.REQUEST) as scope:
            _ = scope.resolve(_Repo[int])

    run_benchmark(benchmark, bench_diwire_enter_close_scope_resolve_open_generic_scoped)


def test_benchmark_dishka_enter_close_scope_resolve_open_generic_scoped(benchmark: Any) -> None:
    provider = Provider(scope=DishkaBenchmarkScope.APP)
    provider.provide(_GenericRepo, provides=_Repo, scope=DishkaBenchmarkScope.REQUEST)
    container = make_dishka_benchmark_container(provider)

    with container(scope=DishkaBenchmarkScope.REQUEST) as first_scope:
        first_int = cast("Any", first_scope.get(_Repo[int]))
        second_int = cast("Any", first_scope.get(_Repo[int]))
        first_str = cast("Any", first_scope.get(_Repo[str]))
    with container(scope=DishkaBenchmarkScope.REQUEST) as second_scope:
        third_int = cast("Any", second_scope.get(_Repo[int]))
    assert isinstance(first_int, _GenericRepo)
    assert isinstance(second_int, _GenericRepo)
    assert isinstance(first_str, _GenericRepo)
    assert isinstance(third_int, _GenericRepo)
    assert first_int is second_int
    assert first_int is not third_int
    assert first_int is not first_str
    assert first_int.dependency_type is int
    assert first_str.dependency_type is str
    assert third_int.dependency_type is int

    def bench_dishka_enter_close_scope_resolve_open_generic_scoped() -> None:
        with container(scope=DishkaBenchmarkScope.REQUEST) as scope:
            _ = scope.get(_Repo[int])

    run_benchmark(benchmark, bench_dishka_enter_close_scope_resolve_open_generic_scoped)
