...
_BASE_HTTP_URL_ENV = "AIOHTTP_E2E_BASE_HTTP_URL"
_BASE_WS_URL_ENV = "AIOHTTP_E2E_BASE_WS_URL"

if os.getenv(_BASE_HTTP_URL_ENV) is None:
    pytest.skip(
        f"{_BASE_HTTP_URL_ENV} is not set; skipping docker-compose aiohttp e2e tests.",
...
try:
    ...
    pytest.skip(
        "websockets is not installed; skipping docker-compose aiohttp e2e tests.",
        allow_module_level=True,
    )


def _base_http_url() -> str:
    return os.getenv(_BASE_HTTP_URL_ENV, "http://api:8230").rstrip("/")


def _base_ws_url() -> str:
    return os.getenv(_BASE_WS_URL_ENV, "ws://api:8230").rstrip("/")
...
@pytest.mark.asyncio
async def test_websocket_request_based_service_returns_ws_path() -> None:
